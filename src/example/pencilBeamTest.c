...
int setup_units(code_units* grackle_units)
{
    ...
    grackle_units->a_value = 1. / (1. + initial_redshift) / my_units.a_units;
...
int initialize_chemistry_fields(int total_field_size, grackle_field_data *grackle_fields, code_units* grackle_units)
{
    ...
    grackle_fields.volumetric_heating_rate = malloc(field_size * sizeof(gr_float));
    ...
    for (i = 0;i < field_size;i++) {
        ...
        my_fields.internal_energy[i] = 1000. / get_temperature_units(grackle_units);
...
int main(int argc, int *argv[])
{
    ...
    // Now assign values
    int field_size[3] = {10,10,1000}; // Number of particles in each dimension
    ...
    if (!(initialize_chemistry_fields(total_field_size, &my_fields, &my_units))){
    ...
    // Run calculation.
    if (!local_solve_chemistry(my_grackle_data, &my_grackle_rates, &my_units, &my_fields, dt)){
    ...
    if (run_calc_cooling_time){
        ...
        for (int i=0; i < number_of_iterations; i++){
            average_elapsed_time += elapsed_times[i]
        }
        average_elapsed_time /= number_of_iterations;
        fprintf(stderr, "**Average time to compute cooling time: %d seconds**\n", average_elapsed_time);
    }
