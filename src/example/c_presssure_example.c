...
// Function to set up the unit system.
int setup_units(code_units *my_units)
{
...
// Function to set up the chemistry.
int setup_chemistry(chemistry_data *my_grackle_data)
{
 ...
 // Function to set up the fields.
int setup_fields(int field_size, grackle_field_data *my_fields)
{
    double tiny_number = 1.e-20; //Constant
...
int main(int argc, char *argv[])
{
  ...
  *********************************************************************/
  ...
  #ifndef _OPENMP
  fprintf(stderr, "SIMPSONS DID IT SIMPSONS DID IT");
  exit(0);
  #endif
  ...
  // These are conversions from code units to cgs.
  code_units my_units;
  if (!setup_units(&my_units)){
      fprintf(stderr, "Failed setting up units!");
      exit(0);
  }
  
  // Second, create a chemistry object for parameters.  This needs to be a pointer.
  chemistry_data *my_grackle_data;
  if (!setup_chemistry(my_grackle_data)){
      fprintf(stderr, "Failed setting up chemistry!");
      exit(0);
  }

  // Create struct for storing grackle field data
  grackle_field_data my_fields;
  int field_size = (int) 1e4; // Set field size.
  if (!setup_fields(field_size, &my_fields)){
      fprintf(stderr, "Failed setting up fields!");
      exit(0);
  }
  ...
  *********************************************************************/
  ...
  // Calculate dust temperature using legacy fortran routine.
  ...
  fprintf(stderr, "dust_temperature = %g K. (FORTRAN)\n", dust_temperature[0]);

  // Calculate dust temperature using new C routine.
  gr_float *dust_temperature_c;
  dust_temperature_c = malloc(field_size, * sizeof(gr_float));
  ...
  fprintf(stderr, "dust_temperature = %g K. (C)\n", dust_temperature_c[0]);
